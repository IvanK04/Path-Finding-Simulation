...
public class AStarService {
    ...
    public void runAStarRealTime(Node startNode, Node endNode, Node[][] nodeGrid, boolean allowDiagonal,
            SseEmitter emitter) {
        int height = nodeGrid.length;
        int width = nodeGrid[0].length;
        int[][] directions = getDirections(allowDiagonal);
        ...
        startNode.setGScore(0);
        startNode.setFScore(allowDiagonal ? HeuristicCalculator.octile(startNode, endNode)
                : HeuristicCalculator.manhattan(startNode, endNode));
        openList.add(startNode);
            ...
            while (!openList.isEmpty()) {
                ...
                for (int[] dir : directions) {
                    ...
                    if (tentativeGScore < neighbor.getGScore()) {
                        ...
                        neighbor.setGScore(tentativeGScore);
                        neighbor.setFScore(
                                tentativeGScore + (allowDiagonal ? HeuristicCalculator.octile(neighbor, endNode)
                                        : HeuristicCalculator.manhattan(neighbor, endNode)));
                        openList.add(neighbor);
                    }
                }
            }
